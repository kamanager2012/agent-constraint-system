...
def _load_scope() -> Dict:
    """v5.3: 统一 scope 读取 — ACTIVE_TASK.json 权威源，SCOPE_FILE 兼容回退。"""
    try:
        # v5.3: ACTIVE_TASK.json is the authoritative source
        ...
        pass
    # v5.3: fallback to SCOPE_FILE for backward compatibility
    try:
...
def _deny(reason: str) -> None:
    ...
    payload = json.dumps({
        "hookSpecificOutput": {
            ...
            "permissionDecisionReason": f"[ACS v5.3] BLOCKED: {reason}",
        }
    })
...
def context_tick() -> Tuple[bool, str]:
    ...
    if state["rounds_since_compact"] >= COMPACT_INTERVAL:
        ...
        return True, f"[ACS v5.3] COMPACT_DUE — {COMPACT_INTERVAL} rounds elapsed. Run /compact to continue."
...
def check_write(file_path: str, tool_name: str, tool_input: Dict) -> Dict:
    ...
    assert resolved.is_relative_to(HOME) or resolved.is_relative_to(Path("/tmp")), \
        f"[ACS v5.3] path escape detected: {resolved}"
    assert scope is not None, "[ACS v5.3] scope must exist at this point"
    ...
    for pattern, desc in COMPILED_BASH:
        if pattern.search(command):
            ...
            if desc in ("inline interpreter", "heredoc interpreter") and current_mode in RESEARCH_MODES:
                print(
                    f"[ACS v5.3] WARNING: [{category}] {desc} — "
...
def cmd_init(args: List[str]) -> None:
    ...
    if not re.match(r"^[a-zA-Z0-9_-]{1,64}$", task_id):
        print(f"[ACS v5.3] ERROR: task_id must match [a-zA-Z0-9_-]{{1,64}}, got: {task_id!r}")
        sys.exit(1)
    ...
    print(f"[ACS v5.3] scope: {task_id} ({len(allowed_dirs)} dirs) "
          f"shadow={shadow} proposal={proposal}")
    print(f"[ACS v5.3] violations cleared, lock released, "
...
def cmd_status() -> None:
    ...
    print(f"[ACS v5.3] task: {tid}  created: {ts_str}  (source: {source})")
    print(f"[ACS v5.3] dirs: {s.get('allowed_dirs', s.get('allowed_files', []))}")
    print(f"[ACS v5.3] shadow: {s.get('shadow_mode', False)} | "
          f"proposal: {s.get('proposal_required', False)}")
    print(f"[ACS v5.3] violations: window={w_score}/{WINDOW_THRESHOLD} "
          f"total={t_score}/{LOCK_DENY_SCORE}")
    print(f"[ACS v5.3] locked: {'YES ⚠' if LOCK_FILE.exists() else 'NO'}")
    print(f"[ACS v5.3] baseline_commands: {len(SCOPE_BASELINE_COMMANDS)} "
    ...
    if not ok:
        for t in tampered:
            print(f"[ACS v5.3]   TAMPERED: {t}")
        for m in missing:
            print(f"[ACS v5.3]   MISSING: {m}")
    else:
        ...
        print(f"[ACS v5.3] INTEGRITY OK ({chain_status})")
...
def cmd_reset(args: List[str]) -> None:
    if "--force" not in args:
        print("[ACS v5.3] ERROR: requires --force flag.")
        sys.exit(1)
    clear_violations(reason="manual_reset_force")
    _budget_reset()
    integrity_store()
    print("[ACS v5.3] violations cleared, budget reset, lock released, baseline updated")
...
def cmd_unlock() -> None:
    if LOCK_FILE.exists():
        try:
            ...
            pass
        print("[ACS v5.3] lock cleared")
    else:
        print("[ACS v5.3] not locked")


def cmd_integrity_check() -> None:
    ok, tampered, missing, new = integrity_verify()
    if ok and not new:
        print("[ACS v5.3] INTEGRITY OK")
        chain_ok, broken = integrity_chain_verify()
        if chain_ok:
            print("[ACS v5.3] chain hash verification: OK")
        else:
            print(f"[ACS v5.3] chain hash verification: BROKEN "
...
def cmd_chain_stats() -> None:
    stats = integrity_chain_stats()
    print(f"[ACS v5.3] chain length: {stats.get('length', 0)}")
    print(f"[ACS v5.3] first snapshot: {stats.get('first_snapshot', '?')}")
    print(f"[ACS v5.3] last snapshot:  {stats.get('last_snapshot', '?')}")
    print(f"[ACS v5.3] first ts: {stats.get('first_ts')}")
    print(f"[ACS v5.3] last ts:  {stats.get('last_ts')}")
    if stats.get("ok"):
        print("[ACS v5.3] chain hash: OK (rolling hash chain verified)")
    else:
        print(f"[ACS v5.3] chain hash: BROKEN "
...
def cmd_chain_verify() -> None:
    ...
    if ok:
        print("[ACS v5.3] chain hash verification: OK (no tamper detected)")
        sys.exit(0)
    print(f"[ACS v5.3] chain hash verification: BROKEN ({len(broken)} broken entries)")
    for b in broken[:10]:
...
def cmd_integrity_store() -> None:
    ...
    print(f"[ACS v5.3] baseline stored: "
...
def main() -> None:
    ...
    #                 f"[ACS v5.3] TOKEN_SOFT: {combined:,}/{TOKEN_SOFT_LIMIT:,} — approaching limit"
    #         }
    #     })
    #     sys.stderr.write(msg + "\n")
    # [DISABLED by user request] TOKEN_HARD check
    ...
    #             "permissionDecision": "warn",
    #             "permissionDecisionReason":
    #                 f"[ACS v5.3] TOKEN_HARD: {combined:,} >= {TOKEN_HARD_LIMIT:,}. "
    ...
    if tool in ("Write", "Edit", "MultiEdit"):
        ...
        if cmd:
            ...
            if result.get("allowed"):
                ...
                if action == "degrade":
                    print(f"[ACS v5.3] Auto-degrade: {budget.get('current_model')} → "
...
_COMMANDS = {
    ...
    "compact-ack":       lambda: (context_compact_done(),
                                   print("[ACS v5.3] compact acknowledged")),
